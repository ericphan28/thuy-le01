-- Fix function create_pos_invoice để sử dụng table invoice_details thay vì invoice_items
-- Script này chỉ sửa phần INSERT invoice items
...
BEGIN
    ...
    -- Insert invoice items using invoice_details table with all required fields
    FOR v_cart_item IN SELECT * FROM jsonb_array_elements(p_cart_items)
    LOOP
        SELECT * INTO v_product FROM products 
        WHERE product_id = (v_cart_item->>'product_id')::integer;
        ...
        INSERT INTO invoice_details (
            ...
            v_line_total - (COALESCE(v_product.cost_price, 0) * (v_cart_item->>'quantity')::numeric),
            p_created_by,
            CASE 
            ...
            END,
            CASE 
                WHEN p_payment_method = 'cash' AND p_invoice_type != 'temp_order' THEN COALESCE(p_paid_amount, 0)
                ELSE 0
            END,
            COALESCE((SELECT u.unit_name FROM units u WHERE u.unit_id = v_product.base_unit_id), 'pcs'),
            CASE 
                WHEN p_invoice_type = 'temp_order' THEN 'pending'
                ELSE 'completed'
            END
